  ...
  return (
    <div>
      ...
      <div className={btn.block}>
        <button className={btn.style} onClick={startTimer} disabled={running || (minutes === 0 && seconds === 0)}>Start</button>
        <button className={btn.style} onClick={stopTimer} disabled={!running}>Stop</button>
        <button className={btn.style} onClick={resetTimer}>Reset</button>
      </div>
    </div>
  );
};
